	...
	*/
    ...
    torcado_init: function () {
        ...
        var newChannelPreviewColor = "#FFFFFF",
            channelSettingsPreviewColor = "#FFFFFF",
            settingsBlogArray = [],
            settingsChannelName = "",
            settingsChannelColor = "";
    ...
    channel_button_detection: function(){
        ...
        $(document).on("mouseenter", ".torcado_addUserChannelButton", function (event) {
            if($(this).closest(".post").length !== 0){
                checkBlog = XKit.extensions.torcado_channels.channels_containing_blog({
                ...
                checkBlog = XKit.extensions.torcado_channels.channels_containing_blog_name($(this).closest(".navigation").find(".name").text());
            } else if($(this).closest(".follower").length !== 0){
                checkBlog = XKit.extensions.torcado_channels.channels_containing_blog_name($(this).closest(".follower").find(".name").text());
            }
        ...
        $("li.post_container").not("#new_post_buttons, #new_post, .torcado_checked").has("div").each(function () {
            ...
            if (!(checkBlog === false)) {
                if (settings["colorPosts"]) {
                    ...
                    $(".post_header", postBody).css({
                        "color": checkBlog[0].color
                    });
                } else {
                    $(postBody).css({
                        "background": "#FFF"
